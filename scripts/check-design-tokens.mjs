...
for (const file of files) {
  ...
  if (colorPattern.test(content) || rawVisualValuePattern.test(content) || rawVisualPropPattern.test(content)) violations.push(file);
...
if (violations.length > 0) console.error(`Цвета, размеры, радиусы и веса должны идти через design tokens:\n${violations.join('\n')}`);
