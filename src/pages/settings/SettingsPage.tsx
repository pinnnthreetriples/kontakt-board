...
import { MaxAccountSettings } from '../../features/max-auth/ui/MaxAccountSettings';
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3 }, maxWidth: tokens.size.contentNarrow }}>
      <Paper sx={{ border: 1, borderColor: 'divider', borderRadius: tokens.radiusCss.lg }}>
        <Tabs aria-label="Разделы настроек" variant="scrollable" scrollButtons="auto" value={tab} onChange={(_, value: number) => setTab(value)} sx={{ px: 2, borderBottom: 1, borderColor: 'divider' }}><Tab label="Этапы" /><Tab label="Поля контакта" /><Tab label="Резервные копии" /><Tab label="Общие" /><Tab label="MAX аккаунт" /></Tabs>
        <Box sx={{ p: { xs: 2, sm: 3 } }}>
